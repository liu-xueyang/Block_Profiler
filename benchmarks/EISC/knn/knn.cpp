...
void read_data(void) {
  int fd = open("/home/xueyang.liu/Block_Profiler/benchmarks/EISC/knn/knn_data.txt", O_RDWR);
...
int main(int argc, char **argv) {
  // ANNOTATE_SITE_BEGIN_WKR("R1_read_data", 0);
  puts("read_data().....");
  Timer timer;
  read_data();
  ...
  // ANNOTATE_SITE_BEGIN_WKR("R2_gen_predicting_data", 0);
  ...
  ANNOTATE_SITE_BEGIN_WKR("R3_calc_distance", 0);
  puts("calc_distance().....");
  timer.reset();
  calc_distance();
  cout << "elapsed time = " << timer.elapsed() / 1000 << endl;
  ...
  ANNOTATE_SITE_BEGIN_WKR("R4_prediction", 0);
  puts("prediction().....");
  timer.reset();
  prediction();
  cout << "elapsed time = " << timer.elapsed() / 1000 << endl;
