...
string TraceFileName = "trace.out.0";
...
VOID PIN_FAST_ANALYSIS_CALL ProfilingEndPoint(void* siteObj)
{
...
// Insert starting and ending function based on the code annotation
VOID Image(IMG img, VOID *v)
{
    ...
    AFUNPTR afunptr(0);
    afunptr = (AFUNPTR)ProfilingStartPoint;
    ...
    // insert end function
    string endSiteName("ANNOTATE_SITE_END_WKR");
    afunptr = (AFUNPTR)ProfilingEndPoint;
    rtn = RTN_FindByName(img, endSiteName.c_str());
    if (!RTN_Valid(rtn)) {
        if (debug) cerr << "ANNOTATE_SITE_BEGIN_WKR is an invalid function" << endl;
        return;
    }
...
VOID Instruction_memTrace(INS ins, VOID* v)
{
...
int main(int argc, char* argv[])
{
    ...
    if (KnobCount)
    {
        ...
        footprint.activate();
        // INS_AddInstrumentFunction(Instruction_memTrace, 0);
